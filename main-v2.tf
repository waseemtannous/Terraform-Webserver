...
# build docker image
resource "docker_image" "server_v2_docker_build" {
  name = var.server_v2_container_name
...
# server docker image v2
resource "docker_registry_image" "server-image-v2" {
  name = docker_image.server_v2_docker_build.name
}

# server-v2 container module
module "gce-advanced-container" {
  source  = "terraform-google-modules/container-vm/google"
  version = "~> 2.0"

  container = {
    image = docker_registry_image.server-image-v2.name
    env = [
      {
        name  = "DB_HOST"
        value = "${google_compute_instance.database.network_interface.0.access_config.0.nat_ip}"
...
# create server-v2 vm template
resource "google_compute_instance_template" "server-v2-template" {
  name         = "server-v2-template"
  machine_type = var.server_template_machine_type
...
resource "google_compute_instance_group_manager" "server-group" {
  ...
  version {
    instance_template = google_compute_instance_template.server-v2-template.id
    name              = "server-image"
  }
